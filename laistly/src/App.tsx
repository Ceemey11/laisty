...
import {
  ...
  sendPasswordResetEmail // Para resetear contraseña
} from 'firebase/auth';
import {
  getFirestore,
  collection,
  ...
  updateDoc,
  arrayUnion,
  arrayRemove, // Para quitar usuarios de listas compartidas
  getDoc,
  getDocs,
  ...
  const handleInvite = async (e) => {
    ...
    try {
      await updateDoc(doc(getListCollectionRef(db), listId), {
        sharedWith: arrayUnion(inviteEmail.trim().toLowerCase())
      });
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {sharedUsers.length > 0 && (
                <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase flex items-center gap-1">
      ...
      {shareModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl relative">
            ...
            {shareStatus === 'removed' && <p className="text-orange-600 text-sm mb-2 flex items-center gap-1"><CheckCircle className="w-4 h-4" /> User removed.</p>}
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  {sharedUsers.map(email => (
                    <div key={email} className="flex items-center justify-between bg-blue-50 px-3 py-2 rounded-lg border border-blue-100">
          ...
          <div className="flex flex-col md:flex-row gap-2">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm text-gray-600"
                placeholder="Due date (optional)"
              />
            </div>
            <button type="submit" disabled={!newTaskName.trim()} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-700 transition disabled:opacity-50 flex items-center gap-2 justify-center">
            ...
            <div key={task.id} className={`bg-white rounded-xl shadow-sm border border-gray-100 group hover:border-blue-200 transition overflow-hidden ${expandedNotesId === task.id ? 'ring-2 ring-blue-500 ring-offset-2' : ''}`}>
              <div className="p-4 flex items-start justify-between">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  <button onClick={() => handleToggleTask(task)} className="p-1 rounded text-gray-300 hover:text-blue-500 hover:bg-blue-50 transition mt-0.5">
                    <Square className="w-6 h-6" />
                  </button>
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-gray-800 break-words">{task.name}</p>
                    ...
                    {dueDate && (
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        {daysRemaining !== null && (
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${daysRemaining < 0 ? 'bg-red-100 text-red-700' :
  ...
  useEffect(() => {
    ...
    const unsubList = onSnapshot(doc(getListCollectionRef(db), listId), (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        setSharedUsers(data.sharedWith || []);
        setListOwner(data.owner || ""); // Set owner
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between sticky top-16 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h2 className="text-xl font-bold text-gray-800 truncate max-w-[200px]">{listName}</h2>
            {sharedUsers.length > 0 && (
              <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-0.5 rounded uppercase flex items-center gap-1 w-fit mt-1">
                <Users className="w-3 h-3" /> Shared
              </span>
            )}
          </div>
      ...
      {shareModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl relative">
            ...
            {shareStatus === 'removed' && <p className="text-orange-600 text-sm mb-2 flex items-center gap-1"><CheckCircle className="w-4 h-4" /> User removed.</p>}
            {shareStatus === 'link-copied' && <p className="text-blue-600 text-sm mb-2 flex items-center gap-1"><CheckCircle className="w-4 h-4" /> Link copied.</p>}
            {shareStatus === 'error' && <p className="text-red-600 text-sm mb-2 flex items-center gap-1"><AlertTriangle className="w-4 h-4" /> Operation failed.</p>}
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4 font-sans w-full">
        ...
        {showForgotPassword && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl relative">
              ...
              <h3 className="font-bold text-lg mb-2 text-gray-800">Reset Password</h3>
              ...
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <input
                  ...
                  required
                />
                {resetStatus === 'success' && <p className="text-green-600 text-sm flex items-center gap-1"><CheckCircle className="w-4 h-4" /> Email sent! Check your inbox.</p>}
                {resetStatus === 'error' && <p className="text-red-600 text-sm">Error sending email. Try again.</p>}
                <button type="submit" className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition">Send Reset Link</button>
              </form>
            </div>
          </div>
        )}
        ...
        <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full text-center border border-gray-100">
          ...
          <h1 className="text-3xl font-extrabold text-gray-800 mb-2">Laistly</h1>
          ...
          <form onSubmit={handleAuth} className="space-y-4 text-left">
            ...
            {!isRegistering && (
              <div className="text-right">
                <button type="button" onClick={() => setShowForgotPassword(true)} className="text-xs text-green-600 hover:underline">Forgot password?</button>
              </div>
            )}
